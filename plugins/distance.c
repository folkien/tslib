...
static struct ts_sample avg_sample;
	...
	if (readed_samples >= 0) {
		...
		for (current_sample = input_samples, i = 0 ; i < readed_samples; i++, current_sample++) {
            if (ts_pressed == 0) {
                ...
                avg_sample.x = current_sample->x;
                avg_sample.y = current_sample->y;
                avg_sample.pressure = current_sample->pressure;
                printf("Reference : %d %d %d\n",avg_sample.x, avg_sample.y, avg_sample.pressure);
            }
            else {
                // Calculating the average
                avg_sample.x = (3*avg_sample.x + current_sample->x)>>2;
                avg_sample.y = (3*avg_sample.y + current_sample->y)>>2;
                avg_sample.pressure = current_sample->pressure;
                printf("Update : %d %d %d\n",avg_sample.x, avg_sample.y, avg_sample.pressure);
            }
